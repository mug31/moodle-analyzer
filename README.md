...
- **`PlantUmlBuilder.php`**: Modul perakit data arsitektur menjadi skrip PlantUML, yang di dalamnya terintegrasi algoritma *filtering* dan *blacklisting* (seperti pengabaian *local dependency* dan kelas utilitas `stdClass`) untuk menjaga kebersihan visual abstraksi.

## Skrip Hasil Rekonstruksi (*PlantUML*)

Skrip mentah berformat `.puml` hasil pemrosesan otomatis dari alat ini dapat diakses langsung pada *root* direktori:
...
Untuk melihat visualisasi penuh dari diagram kelas UML yang telah berhasil di-render menjadi gambar beresolusi tinggi, silakan kunjungi direktori **`diagram/`** di dalam repositori ini. Di sana terdapat cetak biru arsitektur aktual dari masing-masing modul Moodle yang diteliti.
